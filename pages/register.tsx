...
import { pricingAPI, registerAPI } from '../services/api'
  ...
  return (
    <BaseLayout>
      ...
      >
        ...
        {errorResponse?.status && showForm && (
          <Alert mb="12px" status="error">
            <AlertIcon />
            {errorResponse?.data?.message}
          </Alert>
        )}
        {successResponse && showForm && (
          <Alert mb="12px" status="success">
            <AlertIcon />
            {successResponse}
          </Alert>
        )}
        {!showForm && (
          <>
            <RadioGroup onChange={handleChange} value={selectedPricing}>
              {pricing.map((p) => (
                <div key={`pricinggroup-` + p.id}>
                  {p.is_parent && (
                    <Heading mb="12px" as="h2" fontSize="24px">
                      {p.group_name}
                    </Heading>
                  )}
                  <Box
            ...
            <Button
              ...
              colorScheme="orange"
              w="full"
              onClick={() => selectedPricing && setShowForm(true)}
            >
              Pilih
            </Button>
          </>
              ...
              {createArrayWithNumbers(totalMember).map((index) => {
                ...
                return (
                  <div key={`peserta-` + index}>
                    <Heading mb="8px" as="h3" fontSize="14px">
                      Peserta {totalMember > 1 && index + 1}
                    </Heading>
                    <Input
                      isInvalid={errors?.peserta?.[index]?.name}
                      name={`${fieldName}.name`}
                      ref={register({
                        required: {
                          value: true,
                    ...
                    />
                    <Input
                      isInvalid={errors?.peserta?.[index]?.institution}
                    ...
                    />
                    <Input
                      isInvalid={errors?.peserta?.[index]?.address}
                    ...
                    />
                    <Input
                      isInvalid={errors?.peserta?.[index]?.phoneNumber}
                    ...
                    />
                    <Input
                      isInvalid={errors?.peserta?.[index]?.email}
                    ...
                    <InputGroup>
                      <Input
                        ...
                        isInvalid={errors?.peserta?.[index]?.password}
                    ...
                    <InputGroup>
                      <Input
                        ...
                        isInvalid={errors?.peserta?.[index]?.confirmpassword}
